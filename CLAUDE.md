...
| 4 | Season plan | Dated calendar: sowing window, fertilizer timing, irrigation, weed/pest checkpoints, harvest | ✅ DONE (Task 6) — deterministic `build_season_plan` tool (advisor + recommender). Bundled DAE **BAMIS** + BARC FRG calendar snapshot ([backend/app/data/bd_crop_calendar.json](backend/app/data/bd_crop_calendar.json), accessors [backend/app/crop_calendar.py](backend/app/crop_calendar.py)) → pure engine [backend/app/engines/season_plan.py](backend/app/engines/season_plan.py) dates every stage from the establishment event; farmer date overrides the assumed earliest-in-window start; live Open-Meteo 16-day overlay shifts near-term weather-sensitive tasks (records original date + reason); labelled sources + disclaimer, never invents. 5 crops (mustard/wheat/potato/Rabi maize/Boro rice). Design: [docs/SEASON_PLAN.md](docs/SEASON_PLAN.md). TODO: CZIS variety `duration_override` live call; persist plan; pytest gold cases |
